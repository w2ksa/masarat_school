...
export async function storagePut(
  ...
  if (!config) {
    const placeholder = `https://storage.placeholder/${key}`;
    console.warn(`[Storage] No storage configured - returning placeholder URL for: ${key}`);
    return { key, url: placeholder };
  }
...
export async function storageGet(relKey: string): Promise<{ key: string; url: string }> {
  ...
  if (!config) {
    return { key, url: `https://storage.placeholder/${key}` };
  }
