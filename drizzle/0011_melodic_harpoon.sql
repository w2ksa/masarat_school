...
--> statement-breakpoint
ALTER TABLE `students` MODIFY COLUMN `grade` varchar(50) NOT NULL DEFAULT 'الرابع الابتدائي';--> statement-breakpoint
ALTER TABLE `students` MODIFY COLUMN `section` int DEFAULT 1;--> statement-breakpoint
ALTER TABLE `users` MODIFY COLUMN `role` enum('user','teacher','admin') NOT NULL DEFAULT 'user';--> statement-breakpoint
ALTER TABLE `students` ADD `rank` int;--> statement-breakpoint
ALTER TABLE `students` ADD `comment` text;--> statement-breakpoint
...
ALTER TABLE `teacher_votes` ADD CONSTRAINT `teacher_votes_teacherNameId_teacher_names_id_fk` FOREIGN KEY (`teacherNameId`) REFERENCES `teacher_names`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `teacher_votes` ADD CONSTRAINT `teacher_votes_studentId_students_id_fk` FOREIGN KEY (`studentId`) REFERENCES `students`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
